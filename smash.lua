        ...
        if(gameOver == false and gamePause == false) then
            ...
            function initSmash()
                ...
                if (getSeq=="antBoss") then
                    ...
                    if not event.isBombCollision then
                        local antGroup = target.parent
                        timer.performWithDelay(50,function()
                            CreateAnt.new(antGroup,"ant1",smashedAnt)
                        end,math.random(2,5))
                    end
                end
            ...
            elseif getSeq == "frog" then
                score = score + 30
                SoundControl.Smash(getSeq)
            elseif getSeq == "antBoss" and (target.taps == 3 or event.isBombCollision) then
                score = score + 50
                SoundControl.Smash(getSeq)
                initSmash()
                transition.cancel(target)
